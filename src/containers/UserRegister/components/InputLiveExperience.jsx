...
const InputLiveExperience = (props) => {
  ...
  const onClickLiveExperienceType = (v) => {
    setInputValue(v.type)
    props.setLiveExperience(v.id)

    switch(true) {
      case v.id.length === 0:
        setErrorMessage('ライブ出演回数を選択してください')
        setInputError(true)
        break
      default:
    ...
    <Box>
      ...
      <Box display='flex' flexDirection='row' alignItems='center' mt={1} >
        <Button
          ...
          children={inputValue === '' ? 'ライブ経験を選ぶ' : inputValue}
          mt={10}
        />
              ...
              <Box display='flex' flexDirection='column' justifyContent='center' alignItems='center' flexWrap='wrap'>
                {selectorValue ? selectorValue.map((v) => (
                  <Button
                    ...
                    children={v.type}
                    variant='outlined'
                    onClick={() => onClickLiveExperienceType(v)}
                  />
                )) : ''}
              </Box>
            </FormControl>
