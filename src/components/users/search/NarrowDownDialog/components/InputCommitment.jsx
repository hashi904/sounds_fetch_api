...
const InputCommitment = (props) => {
  ...
  return (
    <Box>
      <Box mt={1}>
        <Typography children='意気込み' />
      </Box>
      <Box display='flex' flexDirection='row' alignItems='center' mt={1}>
        <Button
          variant='outlined' 
          onClick={handleClickOpen}
          children={commitmentLabel === '' ? '意気込みを選ぶ' : commitmentLabel}
          mt={10}
        />
        <Dialog open={open} onClose={handleClose}>
          <DialogTitle children='意気込み' />
          <DialogContent>
            <FormControl variant='outlined'>
              <Box display='flex' justifyContent='center' alignItems='center' flexWrap='wrap'>
                {CommitmentType.map((commitment) => (
                  <Button
                    clickable
                    className={classes.button}
                    value={commitment.value}
                    children={commitment.label}
                    variant='outlined'
                    onClick={() => onClickCommitment(commitment)}
                  />
                ))
                }
              </Box>
